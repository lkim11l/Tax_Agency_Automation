...
export default async function ReportsPage({ searchParams }: Props) {
  ...
  const [{ rows }, history] = await Promise.all([loadAllRegistryRows(filters), listReportExports()]);
  ...
  return (
    <>
      <div className="page-heading"><div><h2>Monthly reports</h2><p className="muted">Database snapshot; amounts remain separated by currency.</p></div><Link href="/registry">Open registry</Link></div>
      ...
      <section className="panel">
        <form action={generateReportAction} className="filter-grid">
          ...
          <div className="filter-actions"><button type="submit">Generate XLSX</button></div>
      ...
      <h3 className="section-gap">Exports</h3>
