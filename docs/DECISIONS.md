...
## ADR-019 — Vercel deployment with database-owned pilot jobs

Status: Accepted

Decision:
Deploy the modular Next.js application to Vercel Hobby and retain Supabase as
the hosted database, Auth and private Storage platform. On Hobby, do not
register a Vercel Cron; an administrator starts the mailbox pipeline manually.
Retain the protected cron endpoint so a future Vercel Pro deployment can invoke
it every five minutes. Claim, heartbeat and finish each pipeline run through
service-only PostgreSQL functions with an advisory lock, stale-run recovery,
persisted stages and safe errors.
...
OCR runtime. Hobby avoids a paid-plan dependency before customer approval,
while manual operation remains reliable and auditable. Database ownership gives
